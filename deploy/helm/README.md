...
| Contour | Host |
|---|---|
| dev | cfg.dev.example.com |
| prod | cfg.prod.example.com |
| prod1 | cfg.prod1.example.com |
...
ingress:
  enabled: true
  className: nginx
  host: cfg.dev.example.com
  whitelistSourceRanges: []        # add VPN/office CIDRs to lock down the UI
  tls:
    - hosts: ["cfg.dev.example.com"]
      secretName: cfg-dev-example-com-tls
config:
  oidcIssuer: "https://gitlab.example.com"
  oidcClientId: "REPLACE_DEV_OIDC_CLIENT_ID"
  oidcRedirectUrl: "https://cfg.dev.example.com/auth/callback"
...
matching `cfg.prod[1].example.com`, pin `image.tag`, and bump `replicaCount`.
</details>
...
- `https://cfg.dev.example.com/auth/callback`
- `https://cfg.prod.example.com/auth/callback`
- `https://cfg.prod1.example.com/auth/callback`
